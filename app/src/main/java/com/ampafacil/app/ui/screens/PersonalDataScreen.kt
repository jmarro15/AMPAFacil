...
fun PersonalDataScreen(
    ...
    var role by remember { mutableStateOf("FAMILY") }
    ...
    // Aquí cargamos lo que ya exista para que la familia solo tenga que revisar, completar y guardar.
    LaunchedEffect(uid) {
            ...
            .addOnSuccessListener { userDoc ->
                ...
                db.collection("ampas").document(ampaCode)
                    ...
                    .addOnSuccessListener { memberDoc ->
                        ...
                        // Aquí mantenemos una clave funcional estable para esta familia dentro del AMPA.
                        memberKey = if (member.memberKey.isNotBlank()) {
                            member.memberKey
                        } else {
                            "${ampaCode}_$uid"
                        }

                        // Aquí evitamos que el rol se quede vacío si todavía no estaba grabado.
                        role = if (member.role.isNotBlank()) {
                            member.role
                        } else {
                            "FAMILY"
                        }
    ...
    fun savePersonalData() {
        ...
        val safeMemberKey = memberKey.ifBlank { "${ampaCode}_$uid" }

        // Aquí usamos merge para no pisar otros datos del miembro que no pertenecen a este formulario.
        val payload = hashMapOf<String, Any>(
            "uid" to uid,
            "ampaCode" to ampaCode,
            "memberKey" to safeMemberKey,
            "role" to role.ifBlank { "FAMILY" },
        ...
        // Aquí solo escribimos createdAt la primera vez, para respetar la fecha real de alta.
        if (createdAt == null) {
        ...
        when {
            ...
            ampaCode.isBlank() -> {
                ...
                Column(
                    ...
                    Text(
                        text = "Aquí podemos actualizar los datos del padre, madre o tutor para que la comunicación del AMPA sea más fácil.",
                    ...
                    OutlinedTextField(
                        ...
                        label = { Text("Email adicional del otro tutor") },
                        modifier = Modifier.fillMaxWidth(),
                        singleLine = true
                    )
                    ...
                    // Aquí enseñamos este bloque en solo lectura para no mezclar este formulario con la gestión de hijos.
                    OutlinedTextField(
